...
DOWNLOAD_FOLDER = r'D:\projexp\downloads'  # Updated path to the downloads directory
...
class TimetableOptimizer:
    ...
    # Load the availability with the new format
    def load_availability(self):
        availability = {}
        for index, teacher in enumerate(self.data['Teacher']):
            avail = self.data['Availability'][index]
            # Store availability directly as a dictionary of days and periods
            availability[teacher] = avail
        return availability
        ...
        for section in range(1, self.sections + 1):
            for day in self.days:
                for period in range(1, self.periods + 1):
                    teacher = self.select_teacher(day, period)
                    if teacher:
                        subject = random.choice(self.data['Qualified'][self.data['Teacher'].index(teacher)])
                        timetable[section][(day, period)] = (subject, teacher)
        return timetable

    # Updated select_teacher to handle the new availability format
    def select_teacher(self, day, period):
        available_teachers = [
            teacher for teacher, avail in self.availability.items()
            if day in avail and period in avail[day]
        ]
        ...
        for section in timetable:
            for (day, period), (subject, teacher) in timetable[section].items():
                ...
                if self.naive_bayes_model.predict([feature])[0] == 1:  # Conflict found
                    new_teacher = self.select_teacher(day, period)
                    if new_teacher:
...
def generate_timetable():
    ...
    with open(timetable_file_path, 'w') as timetable_file:
        timetable_file.write('Section,Day,Period,Subject,Teacher\n')
        for section, schedule in timetable.items():
            for (day, period), (subject, teacher) in schedule.items():
                timetable_file.write(f'{section},{day},{period},{subject},{teacher}\n')

    # Render timetable.html with the timetable, conflicts, and score
    return render_template('timetable.html', timetable=timetable, conflicts=conflicts, score=score)
